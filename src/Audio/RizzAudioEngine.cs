...
#pragma warning restore CS0067
        ...
        {
            _logger.LogInformation("Playing test tone: freq={Freq}Hz, duration={Dur}s on deck {Deck}", frequency, duration, deckNumber);

            if (_mixer == null) return;

            // Create sine wave provider
            var sineWave = new SineWaveProvider(frequency, duration, 0.75f);
            _mixer.SetProvider(0, sineWave);
            _logger.LogInformation("Sine wave provider created: {Freq}Hz, {Dur}s, {Vol}%", frequency, duration, 0.75f * 100);
            File.AppendAllText("debug.log", $"{DateTime.Now}: Sine wave created\n");
            ...
            if (devices.Any() && _mixer != null)
            {
                ...
                _logger.LogInformation("Test tone playback started");
                File.AppendAllText("debug.log", $"{DateTime.Now}: Test tone started\n");
            }
            else
            {
